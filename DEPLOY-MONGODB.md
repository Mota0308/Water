...
## Railway（專案 `friendly-surprise`／服務 `desirable-balance`）

**Root Directory 必須是 repo 根目錄**（同時有 `package.json`、`store-web/`、`server/`），不要選 `store-web` 或 `server` 子資料夾。  
Start command：`npm start`（已寫在根目錄 `package.json` 與 `railway.toml`）。
